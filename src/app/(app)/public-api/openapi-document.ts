    ...
    paths: {
      "/api/v1/ita/{year}": {
        get: {
          ...
          description:
            "คืนอาร์เรย์ของ ITA ปีนั้น เรียงตามลำดับที่เจ้าหน้าที่จัดไว้ (order) แต่ละรายการมี `oits` (รายการย่อย) เรียงตาม id ตามระบบเดิม\n\n**ปีที่ไม่มีข้อมูล → `200` + `[]`** ไม่ใช่ 404 — เว็บคณะไล่ปีถอยหลังเพื่อหาปีล่าสุดที่มีข้อมูล และถือว่า response ที่ไม่ใช่อาร์เรย์คือข้อผิดพลาดร้ายแรง\n\n**ข้อยกเว้น rate limit:** คำขอที่ browser ส่ง header `Sec-Fetch-Site: same-origin` (เช่นหน้าแรกของเว็บนี้เอง) จะไม่ถูกนับเป็น `429` ถึงจะเกินโควตา — เพราะผู้เยี่ยมชมหลายคนอาจอยู่หลัง NAT เดียวกัน ยังคงนับ/แสดง header ตามปกติ แต่ consumer ภายนอก (เว็บคณะ) ไม่ได้ส่ง header นี้จึงไม่เข้าเงื่อนไข",
